...
The later `STM32H735ZGT6` branch adds a dedicated target for the pin-compatible crypto-enabled STM32H735ZGT6. It reuses the same PCB signal map, USB configuration, memory layout, and selectable one/two/three-channel FDCAN partition, but selects the correct `STM32H735xx` CMSIS header, H735 startup vector table, board identity, and programmer device. Its hardware assumptions and build flags are documented in [doc/README.stm32h735zgt6.rst](doc/README.stm32h735zgt6.rst). `STM32H735ZTG6` is not a valid ST ordering code.
