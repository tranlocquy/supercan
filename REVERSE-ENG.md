...
The `2FDCAN` branch extends that custom target with a selectable FDCAN count. The later `3FDCAN` branch accepts `STM32H725_FDCAN_COUNT=1|2|3`: channel 0 uses FDCAN1 on PB8/PB9, optional channel 1 uses FDCAN2 on PB5/PB6, and optional channel 2 uses FDCAN3 on PG10/PG9 (RX/TX). All enabled controllers share the STM32H725's fixed 10 KiB CAN message RAM, so the three-channel build uses smaller per-controller hardware transmit FIFOs.

The later `STM32H735ZGT6` branch adds a dedicated target for the pin-compatible crypto-enabled STM32H735ZGT6. It reuses the FDCAN and USB signal map, memory layout, and selectable one/two/three-channel FDCAN partition, but selects the correct `STM32H735xx` CMSIS header, H735 startup vector table, board identity, and programmer device. The H735 target uses active-low, open-drain LEDs on PE2 (blue/debug), PE3 (green CAN0 status), and PE4 (red CAN0 status). PE2 turns on as the first board-specific action in `main()`, before board, clock, and FDCAN initialization, and remains a steady main-reached indicator; USB and CAN activity do not override it. The target deliberately leaves the legacy USART3 pins PD8/PD9, LED pins PB0/PB14/PE1, and button pin PC13 unconfigured. Its hardware assumptions and build flags are documented in [doc/README.stm32h735zgt6.rst](doc/README.stm32h735zgt6.rst). `STM32H735ZTG6` is not a valid ST ordering code.

### Optional external clock on the later branch

The later custom target also accepts `STM32H725_USE_HSE=0|1`; the exact default is `0`, which retains the internal HSI64 clock path. An opt-in two-channel crystal build is produced with `make BOARD=stm32h725zgt6 STM32H725_USE_HSE=1`. HSE outputs are isolated from HSI objects: the one-, two-, and three-channel directories are `_build/stm32h725zgt6-fdcan1-hse25`, `_build/stm32h725zgt6-hse25`, and `_build/stm32h725zgt6-fdcan3-hse25`. The release script explicitly uses `STM32H725_USE_HSE=0` and continues to package the two-channel HSI image from `_build/stm32h725zgt6`.

The H735 target mirrors those controls as `STM32H735_FDCAN_COUNT=1|2|3` and `STM32H735_USE_HSE=0|1`, with the same defaults and output suffixes under `_build/stm32h735zgt6*`. Its packaged release image is likewise the explicit two-channel HSI configuration.

The HSE option is crystal mode (`RCC_HSE_ON`, not bypass) for a 25 MHz crystal or resonator between PH0-OSC_IN (LQFP144 pin 25) and PH1-OSC_OUT (pin 26). Select and lay out the resonator/load network using the component data and ST's [AN2867 oscillator design guide](https://www.st.com/resource/en/application_note/an2867-oscillator-design-guide-for-stm8afals-stm32-mcus-and-mpus-stmicroelectronics.pdf); parasitic capacitance and crystal parameters determine the load components, so no universal capacitor value is specified here. The HSE calculations are `25 MHz / 5 * 48 / 2 = 120 MHz` for PLL1 P/SYSCLK and `25 MHz / 5 * 48 / 3 = 80 MHz` for the PLL2 Q kernel clock, both using a 5 MHz input and 240 MHz wide-range VCO. The shared CCU divides the FDCAN kernel clock by two, producing a 40 MHz time-quanta clock that is strictly below the 60 MHz APB1 clock. HSI48 remains the USB source.

Clock initialization runs before the BSP starts a HAL tick source. If the crystal is missing or does not start, the HAL can remain in its HSE-ready polling loop and firmware never reaches USB or SuperCAN. A HAL-reported oscillator or PLL error enters the BSP failure loop. There is no automatic HSE-to-HSI fallback.
