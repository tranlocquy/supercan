...
The later `STM32H735ZGT6` branch adds a dedicated target for the pin-compatible crypto-enabled STM32H735ZGT6. It reuses the FDCAN and USB signal map, memory layout, and selectable one/two/three-channel FDCAN partition, but selects the correct `STM32H735xx` CMSIS header, H735 startup vector table, board identity, and programmer device. The H735 target uses active-low, open-drain LEDs on PE2 (blue/debug), PE3 (green CAN0 status), and PE4 (red CAN0 status). It deliberately leaves the legacy USART3 pins PD8/PD9, LED pins PB0/PB14/PE1, and button pin PC13 unconfigured. Its hardware assumptions and build flags are documented in [doc/README.stm32h735zgt6.rst](doc/README.stm32h735zgt6.rst). `STM32H735ZTG6` is not a valid ST ordering code.
